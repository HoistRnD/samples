...
(function () {
	...
	extend(ConnectorManager.prototype, {
		...
		connect: function (url, success, error, redirect, context) {
			...
			var _success = function(res) {
				if(res.redirect) {
					if(redirect) {
						redirect.apply(self, [res.redirect]);
					} else {
			...
			request(this.hoist._configs, { url: this.url + "/connect", data: { return_url: url } }, _success, error, context);
		},
